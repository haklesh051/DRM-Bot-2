...
# Config 
class Config(object):
    BOT_TOKEN = os.environ.get("BOT_TOKEN", "7633147515:AAGehPj6nS4bw72beMusMbOuyDRLFi5cdjY")
    API_ID = int(os.environ.get("API_ID",  "23977225"))
    API_HASH = os.environ.get("API_HASH", "969c399e6aea5032314a2e17cab428be")
    DOWNLOAD_LOCATION = "./DOWNLOADS"
    SESSIONS = "./SESSIONS"

    AUTH_USERS = os.environ.get('AUTH_USERS', '7681589139').split(',')
    for i in range(len(AUTH_USERS)):
        AUTH_USERS[i] = int(AUTH_USERS[i])

    GROUPS = os.environ.get('GROUPS', '-100280874428').split(',')
    for i in range(len(GROUPS)):
        GROUPS[i] = int(GROUPS[i])

    LOG_CH = os.environ.get("LOG_CH", "-1002863059177")
